...
<?php
    ...
    if(isset($_POST['btnSave'])){
        ...
        $insert_data = mysqli_query($conn, "INSERT INTO learningoutcometbl (learningOutcomeCode,courseOutcomeCode,learningOutcomeDesc,timeFrame)
            VALUES ('$checkcode','$scode','$pcode','$tf')");

        if($insert_data){
            header('Location:learningoutcome.php?message=successadd');
        }
        else{
...
<?php
    ...
    if(isset($_POST['btnUpdate'])){
        ...
        mysqli_query($conn, "UPDATE learningoutcometbl SET learningOutcomeCode='$checkcode', courseOutcomeCode='$scode', learningOutcomeDesc='$pcode', timeFrame='$tf'
            WHERE learningOutcomeID=$id");

        header('location: learningoutcome.php?message=successupdate');
    }
?>
...
<?php
    ...
    if(isset($_GET['delete'])){
        ...
        mysqli_query($conn, "DELETE FROM learningoutcometbl WHERE learningOutcomeID=$id");
        header('location: learningoutcome.php?message=successdelete');
    }
?>
                ...
                <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                    ...
                    <li class="nav-item dropdown">
                        ...
                        <ul class="dropdown-menu" aria-labelledby="navbarDropdown" >
                            ...
                            <li><a class="dropdown-item active" href="learningoutcome.php">Learning Outcome</a></li>
                ...
                <div class="alert alert-success text-center" role="alert" id="alert">
                    <?php 
                        if($_GET['message'] == "successadd"){
                            echo "Succesfully Added Course Outcome";
                        }
                        elseif($_GET['message'] == "successupdate"){
                            echo "Successfully Updated Course Outcome";
                        }
                        elseif($_GET['message'] == "successdelete") {
                            echo "Succesfully Deleted Course Outcome";
                        }
                    ?>
                </div>
            <?php endif ?>
                    ...
                    <table class="table table-borderless table-hover table-fixed">
                        <thead>
                            <tr style="width:100%">
                                ...
                                <th style="width:20%">Learning Outcome Code</th>
                                <th style="width:36%">Course Outcome Code</th>
                                <th style="width:40%">Learning Outcome Description</th>
                                <th style="width:40%">Time frame</th>
                                <th style="width:4%">Actions</th>
                            </tr>
                        </thead>
                        <?php
                            $select_data = mysqli_query($conn, "SELECT * FROM learningoutcometbl");
                            while($program = mysqli_fetch_array($select_data)){
                        ?>
                        <tbody style="text-align:center;" class="table-primary">
                            <tr style="width:100%">
                                <td style="font-size:0px;" class="id"><?php echo $program['learningOutcomeID']?></td>
                                <td class="clcode"><?php echo $program['learningOutcomeCode']?></td>
                                <td class="subcode"><?php echo $program['courseOutcomeCode']?></td>
                                <td class="pccode"><?php echo $program['learningOutcomeDesc']?></td>
                                <td class="tf"><?php echo $program['timeFrame']?></td>
                                <td>
                                    <a href="#editModal" class="edit" id="btnEditModal" name="btnEditModal" data-toggle="modal" style="text-decoration:none;">
                                        <i class="far fa-edit"></i>
                                    </a>
                                    <a href="learningoutcome.php?delete=<?php echo $program['learningOutcomeID']?>" class="delete" name="btnDelete">
                ...
                <div class="modal-content">
                    <form method="POST" action="#">
                        <div class="modal-header">
                            <h4 class="modal-title">Add Learning Outcome</h4>
                            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">Close</button>
                        </div>
                        <div class="modal-body">
                            <div class="form-group">
                                <label>Learning Outcome Code</label>
                                <input name="checkcode" class="form-control" required></input>
                            </div>
                            <div class="form-group">
                                <label>Course Outcome Code</label>
                                <select name="subcode" class="form-control" required>
                                <?php
                            $select_data = mysqli_query($conn, "SELECT * FROM courseoutcometbl");
                            while($Subject = mysqli_fetch_array($select_data)){
                        ?>
                                <option value="<?php echo $Subject['courseOutcomeCode']?>"><?php echo $Subject['courseOutcomeCode']?></option>
                                <?php }; ?>
                                </select>
                            </div>
                            <div class="form-group">
                                <label>Learning Outcome Description</label>
                                <textarea name="pcCode" class="form-control" required>
                ...
                <div class="modal-content">
                    <form method="POST" action="#">
                        ...
                        <div class="form-group">
                                ...
                                <input id="clcode" name="checkcode" class="form-control" required></input>
                            </div>
                            <div class="form-group">
                                <label>Course Outcome Code</label>
                        ...
                        ?>
                            ...
                            <div class="form-group">
                                <label>Learning Outcome Description</label>
                                <textarea id="pccode" name="pcCode" class="form-control" required></textarea>
                            </div>
                            <div class="form-group">
                                <label>Time Frame</label>
                                <select id="tf" name="tf" class="form-control" required>
                                <option value="1">1</option>
                                <option value="2">2</option>
                                <option value="3">3</option>
                                </select>
